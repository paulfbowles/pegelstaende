...
Umgebungsvariablen (GitHub Secrets):
  DATAWRAPPERPEGELKARTE / DW_API_KEY   – Datawrapper API-Schluessel
...
# ---------------------------------------------------------------------------
# 3. Filtern – nur RHEIN, mit Koordinaten und Messwert
# ---------------------------------------------------------------------------

def filtere(df_all: pd.DataFrame) -> pd.DataFrame:
    mask = (
        (df_all["gewaesser"].str.upper() == "RHEIN")
        & df_all["lat"].notna()
        & df_all["lon"].notna()
        & df_all["wasserstand_cm"].notna()
    )
    df = df_all[mask].copy().reset_index(drop=True)
    print(f"[{_now()}] Nach Filter (nur RHEIN): {len(df)} Stationen.")
    return df
...
if __name__ == "__main__":
    try:
        ...
        df     = filtere(df_all)
        df     = bereichere(df)
        # Nur Stationen mit bekanntem Status behalten
        df     = df[df["status_label"] != "Unbekannt"].reset_index(drop=True)
        print(f"[{_now()}] Nach Status-Filter: {len(df)} Stationen.")
